# result: 74984 rounds: 52 hitpoints: 1442 elfpower: 19
#
...
def attack(row, col):
    ...
    if isElf(unit):
        hitPoints[chosenUnit] -= elfPower
    else:
        hitPoints[chosenUnit] -= goblinPower
...
def playAGame(elfPower):
    # return True if elves win without losing an elf
    cavern, hitPoints, elfStarts = createCavern(elfPower)
    ...
    for rounds in range(250):
        ...
        for row, col in unitOrder:
            ...
            if len(elves) == 0 or len(goblins) == 0:
                ...
                #return len(elves) if len(elves) > 0 else -1
                return len(elves) == elfStarts

        printCavern()

def createCavern(epower):

    global cavern, hitPoints
    hitPoints = {}
    elfPower = epower

    cavern = {(r,c): lines[r][c] for r in range(nrows)for c in range(ncols)}
    elves = 0
    gIndex = 0
    eIndex = 0
    for r in range(nrows):
...
if __name__ == "__main__": 
    ...
    while not elfWin:
        elfPower += 1
        elfWin = playAGame(elfPower)
        print 'played game with elfpower', elfWin, elfPower